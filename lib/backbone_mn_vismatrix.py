...
import pdb
import math
from .draw_attention import visulize_attention, visulize_attention_lan
...
class MultiModalSwinTransformer(nn.Module):
    ...
    def forward(self, x, l, l_mask, filename, sentence):
        """Forward function."""
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, l_new, H, W, x, Wh, Ww = layer(x, Wh, Ww, l, l_mask, filename, sentence)
            l = l_new
...
class MMBasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, l, l_mask, filename, sentence):
        """ Forward function.
        ...
        # PWAM fusion
        x_residual, l_residual = self.fusion(x, l, l_mask, filename, sentence)
...
class PWAM(nn.Module):
    ...
    def forward(self, x, l, l_mask, filename, sentence):
        ...
        lang, lang1 = self.image_lang_att(x, l, l_mask, filename, sentence)  # (B, H*W, dim) (B, l_dim, N_l)
...
class SpatialImageLanguageAttention(nn.Module):
    def __init__(self, v_in_channels, l_in_channels, key_channels, value_channels, out_channels=None, num_heads=1):
        ...
        self.dy_conv = nn.Sequential(
            Conv_DCFD(self.v_in_channels, self.v_in_channels, kernel_size=3, inter_kernel_size=5, padding=1, stride=1,
                      bias=True),  # the init function sets bias to 0 if bias is True
            nn.BatchNorm2d(self.v_in_channels),
            nn.ReLU()
            )
...
class SpatialImageInteraction(nn.Module):
    ...
    def forward(self, x, l, l_mask, filename, sentence):
        ...
        sim_mapv11 = self.refineimg11(x, l, l_mask, filename, sentence)
        sim_mapv33 = self.refineimg33(x, l, l_mask, filename, sentence)
        sim_mapv55 = self.refineimg55(x, l, l_mask, filename, sentence)
        sim_mapv = (sim_mapv11 + sim_mapv33 + sim_mapv55) / 3 # (B, num_heads, h*w, N_l)

        length = torch.sum(l_mask)
        visulize_attention(sim_mapv, length, filename, sentence)
        ...
        # pdb.set_trace()
        sim_mapl = (sim_mapl11 + sim_mapl21 + sim_mapl31) / 3 # (B, self.num_heads, N_l, H*W)

        length = torch.sum(l_mask)
        visulize_attention_lan(sim_mapl, length, filename, sentence)
...
#########################################################################
        ...
        # x1 = self.f_query(x)
        # x1 = rearrange(x1, 'b c (h w) -> b c h w', h=int(math.sqrt(x.shape[2])))
        # x1 = F.unfold(x1, kernel_size=self.kernel, stride=1, padding=self.kernel//2)
        # x2 = rearrange(x1, 'b c h w -> b c (h w)')
        #
        # query = self.f_query2(x2)  # (B, key_channels, H*W) if Conv1D
        # query = query.permute(0, 2, 1)  # (B, H*W, key_channels)
