...
class MultiModalSwinTransformer(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, l_new, H, W, x, Wh, Ww = layer(x, Wh, Ww, l, l_mask)
            l = l_new
...
class RefineVisualSim(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        ...
        sum0 = (0 + (HW-1))*HW/2
        new_mask = torch.ones_like(sim_map).cuda()
        ...
        for i in range(sim_map.shape[0]):
            ...
            index1 = torch.sort(index0, dim=0)[1]
            index2 = (index1 / sum0).unsqueeze(0)
            new_mask[i, :, :, :len0] = index2
