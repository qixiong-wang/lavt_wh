    ...
    def forward(self, x, l, l_mask):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                rmi_loss.append(RMI_loss(xp, x_out))
                # pdb.set_trace()

                out = x_out.view(-1, H, W, self.num_features[i]).permute(0, 3, 1, 2).contiguous()
                outs.append(out)

        return tuple(outs), torch.sum(rmi_loss[0] + rmi_loss[1] + rmi_loss[2] + rmi_loss[3] )
...
class PWAM(nn.Module):
    def __init__(self, dim, v_in_channels, l_in_channels, key_channels, value_channels, num_heads=0, dropout=0.0):
        ...
        self.dy_conv = nn.Sequential(Conv_DCFD(dim, dim,kernel_size=3, inter_kernel_size=5, padding=1, stride=1, bias=True),  # the init function sets bias to 0 if bias is True
                                         nn.BatchNorm2d(dim),
                                         nn.ReLU()
                                        )
