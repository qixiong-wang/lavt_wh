...
class SimpleDecoding(nn.Module):
    def __init__(self, c4_dims, factor=2):
        ...
        self.conv1_3 = nn.Conv2d(hidden_size + c2_size + 1, hidden_size, 3, padding=1, bias=False)
        ...
        self.conv1_2 = nn.Conv2d(hidden_size + c1_size + 1, hidden_size, 3, padding=1, bias=False)
    ...
    def forward(self, x_c4, x_c3, x_c2, x_c1):
        ...
        if pre1.size(-2) < x_c2.size(-2) or pre1.size(-1) < x_c2.size(-1):
            pre11 = F.interpolate(input=pre1, size=(x_c2.size(-2), x_c2.size(-1)), mode='bilinear', align_corners=True)
        pre11 = self.softmax(pre11)[:, 1, :, :].view(pre11.shape[0], 1, pre11.shape[2], pre11.shape[3])
        pre12 = pre11 * (pre11>0.7)
        ...
        x = torch.cat([x, x_c2, pre12], dim=1)
        x = self.conv1_3(x)
        x = self.bn1_3(x)
        x = self.relu1_3(x)
        x = self.conv2_3(x)
        ...
        if pre2.size(-2) < x_c1.size(-2) or pre2.size(-1) < x_c1.size(-1):
            pre21 = F.interpolate(input=pre2, size=(x_c1.size(-2), x_c1.size(-1)), mode='bilinear', align_corners=True)
        pre21 = self.softmax(pre21)[:, 1, :, :].view(pre21.shape[0], 1, pre21.shape[2], pre21.shape[3])
        pre22 = pre21 * (pre21>0.7)
        ...
        x = torch.cat([x, x_c1, pre22], dim=1)
        x = self.conv1_2(x)
        x = self.bn1_2(x)
        x = self.relu1_2(x)
        x = self.conv2_2(x)
