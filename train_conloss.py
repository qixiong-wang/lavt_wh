        ...
        for data in metric_logger.log_every(data_loader, 100, header):
            ...
            for j in range(sentences.size(-1)):
                if bert_model is not None:
                    ...
                    loss_contra, output = model(image, embedding, l_mask=attentions[:, :, j].unsqueeze(-1))
                else:
...
def main(args):
    ...
    # resume training (optimizer, lr scheduler, and the epoch) and also add best performance
    if resume_flag:
