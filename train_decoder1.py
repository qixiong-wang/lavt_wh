    ...
    with torch.no_grad():
        for data in metric_logger.log_every(data_loader, 100, header):
            ...
            if bert_model is not None:
                ...
                pre1, pre2, output = model(image, embedding, l_mask=attentions)
            else:
...
def train_one_epoch(model, criterion, optimizer, data_loader, lr_scheduler, epoch, print_freq,
    ...
    train_loss = 0
    total_its = 0
    adp60 = nn.AdaptiveAvgPool2d((60,60))
    adp120 = nn.AdaptiveAvgPool2d((120,120))
    ...
    for data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        loss60 = criterion(pre1, target)
        loss120 = criterion(pre2, target)
        loss = loss0 + loss60 + loss120
