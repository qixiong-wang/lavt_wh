...
outputfile = root_path + 'train_notrefall_cap_mustisare_begin234567s'
# outputfile = './'
...
save_num = 0
used_image = []
check_list = ['Two', 'Three', 'Four', 'Five', 'Six', 'Seven', 'Several']

ff = open(root_path + 'train_notrefall_cap_mustisare_begin234567s.csv', 'w')
ff1 = csv.writer(ff)
...
for i in range(length):
    ...
    if filename_now not in all_refcoco_pools:
        ...
        # if cap_now[0] != 'a' and cap_now[0] != 'A':
        # pdb.set_trace()
        ...
        # if cap_now[:3] == 'Two' or cap_now[:5] == 'Three' or cap_now[:4] == 'Four' or cap_now[:4] == 'Five' or cap_now[:7] == 'Several':
            # pdb.set_trace()
            new_filename = '2_' + str(save_num+1) + '_' + filename_now.split('.')[0] + '_' + cap_now  + '_' + '.jpg'
