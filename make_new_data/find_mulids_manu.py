...
def main(argv):
    ...
    jsonfile = '/data/huyutao/new_coco/from_gongsi/out12.12.json'    # './data/coco/annotations/instances_val2017.json'
    ...
    refseg_train_pools_wo_test = np.load('/data/huyutao/new_coco/dataset_split_true_round2/refseg_train_pools_wo_test.npy')

    mkdir_os(outputfile)

    coco_clas = np.load('/data/huyutao/new_coco/coco2014_cla_ids.npy', allow_pickle=True).item()

    # ff = open('/data/huyutao/new_coco/dataset_split_true_round2/train_wotest_multar_cla_name1.csv', 'w')
    # ff1 = csv.writer(ff)
    ...
    for i, imgId in enumerate(imgIds):
        ...
        cat_id_area = {}
        cat_small = {}
        # if img['file_name'] in refseg_train_pools_wo_test:
        if True:
        # if img['file_name'] == 'COCO_train2014_000000000144.jpg':
...
##########################ori_vis_related###############################
            ...
            area_bar = img['height'] * img['width'] * 0.035
            for j in range(len(anns)):
                ...
                if id_now not in cat_id_fre:
                    ...
                    if area_now > area_bar:
                        cat_small[id_now] = False
                    else:
                        cat_small[id_now] = True
                else:
            ...
            # pdb.set_trace()
            # if maxarea_fre > 1 and maxarea_fre < 7  and not cat_small[maxarea_id]:
            if True:
                use_num = use_num + 1
                ...
                cv2.imwrite(outputfile + img_file_name, final)
