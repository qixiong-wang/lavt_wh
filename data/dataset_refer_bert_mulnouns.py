...
class ReferDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self.chuqu = ['in', 'on', 'with',  'first', 'second', 'i']
        ...
        for r in ref_ids:
            ...
            for i, (el, sent_id) in enumerate(zip(ref['sentences'], ref['sent_ids'])):
                ...
                for word_i, (word_now, chara) in enumerate(chara_list):
                    # if (chara == 'NN' or chara == 'NNS') and word_i < 19:
...
###########################Mask_Multiple_Nouns#################################
                    ...
                    # select_length = choice(np.arange(1, (len(mask_ops)+1 // 2) + 1))
                    # could_mask = sample(mask_ops, select_length)
                    # attention_remask = copy.deepcopy(attention_mask)
                    # # print([sentence_raw], mask_ops1, [could_mask], attention_mask)
                    # could_mask = (np.array(could_mask) + 1).tolist()
                    # attention_remask = np.array(attention_remask)
                    # attention_remask[could_mask] = 0
                    # attention_remask = attention_remask.tolist()
                    # input_ids1 = copy.deepcopy(input_ids)
                    # input_ids1 = np.array(input_ids1)
                    # input_ids1[could_mask] = 0
                    # input_ids1 = input_ids1.tolist()
                    # # print(len(mask_ops), select_length, could_mask, input_ids1)
##################################################################
