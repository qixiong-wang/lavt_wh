...
srun --partition=Gvadapt --mpi=pmi2 --gres=gpu:2 -N 1  --job-name=SEG1 --kill-on-bad-exit=1 --pty --quotatype=spot --mail-type=ALL \
python -m torch.distributed.launch --nproc_per_node 2 --master_port 23333 train_conloss.py --model lavt --dataset refcoco \
--model_id debug --batch-size 8 --lr 0.00005 --wd 1e-2 --swin_type base --pretrained_swin_weights ./pretrained_weights/swin_base_patch4_window12_384_22k.pth \
