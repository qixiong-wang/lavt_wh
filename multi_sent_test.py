    ...
    with torch.no_grad():
        for data in metric_logger.log_every(data_loader, 100, header):
            ...
            for j in range(sentences.size(-1)):
                outputs = []
                sentence_len = int(torch.where(attentions[:,:,j]==1)[1][-1])

                if bert_model is not None:
                    # for k in range(sentence_len):
                    #     input_sentence = sentences[:, :, j]
                    #     input_mask = attentions[:, :, j]
                    #     input_sentence=torch.cat((sentences[:, :k+1, j],sentences[:, k+2:, j], torch.zeros(1,1).long().cuda()),dim=1)
                    #     input_mask=torch.cat((sentences[:, :k+1, j],sentences[:, k+2:,j], torch.zeros(1,1).long().cuda()),dim=1)

                    #     last_hidden_states = bert_model(input_sentence, attention_mask=input_mask)[0]
                ...
                output = torch.max(torch.cat(outputs,dim=0),dim=0,keepdim=True)[0]
